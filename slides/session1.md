...
## Photoplethysmography (PPG, 光電容積脈波)
...
透過型のほうが内部の情報が多いので精度が高い, が装着できる部位が限られる
...
- 1つ目のピーク (systolic peak) は心臓の収縮で血液が動脈に流れ込む時のピーク
- 2つ目のピーク (diastolic peak) は血液が末梢や分岐点で反射して戻ってくる時のピーク

</div>
</div>

- 受光量 = 大きな**DC成分** (組織・静脈などの一定吸収) + 小さな**AC成分** (動脈の拍動)
- ACはDCの**約1%程度** ─ 「この小波をどう取り出すか」が第2回で扱う主な課題
...
- **緑(～530nm): 心拍計**
  Hb/HbO$_2$ともよく吸収 → 拍動信号が大きく**SNRが高い**
- **赤(660)＋赤外(940): 酸素飽和度**
  赤では Hb > HbO$_2$, 赤外では逆転 → **2波長の比**から酸素飽和度を算出
...
センサはアナログ値をそのまま出力するか, I$^2$C/SPIでデータをデジタル通信するものが多い
...
- **GPIO** (General Purpose I/O)
  ...
  例: LEDを光らせる, ボタンのOn/Offを読む

- **ADC** (Analog-to-Digital Converter)
  連続的な**アナログ電圧**を**数値**に変換
  例: センサの0-3.3Vを0-4095の整数 (12bit) に
...
※  ログインは不要ですが, コードを保存したい場合はログインが必要
...
オームの法則 $V = IR$ より, 赤色LED (順電圧$V_f \approx 2.0$V)を3.3Vで光らせると:
$$ I = \frac{V_{cc}-V_f}{R} \approx \frac{3.3-2.0}{220} = 5.9\,\text{mA} $$

> 抵抗値を変えると電流=明るさが変わる
> 定格電流はLEDの仕様書を確認 (一般的には20mA程度)
...
- 3.3VとGNDの間につなぎ, 中点を**ADCピン**へ
- つまみを動かす → ADCで読む値が変わる
    $$ V_\text{out} = 3.3V \times \frac{R_2}{R_1 + R_2} $$
...
sensor = ADC(Pin(26)) # ADCに使えるのはGP26,27,28ピンのいずれかのみ
...
```
